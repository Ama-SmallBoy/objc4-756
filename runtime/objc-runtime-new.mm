...
{
    if (!cats) return;//是否有分类，如果没有直接return;
    if (PrintReplacedMethods) printReplacements(cls, cats);

    bool isMeta = cls->isMetaClass();//判断是哪一种分类？NO 为实例方法  YES 为类方法
    ...
    /*
     ...
     [[method_t,method_t],[method_t],[method_t,method_t,...],...],method_list_t存储结构
    ...
    //使用倒序遍历的方式，获取分类方法类表，最先访问最后编译的分类。
    while (i--) {
        
        //获取一个分类：最后编译的分类中的方法会最终生效。
        auto& entry = cats->list[i];
        //获取相应分类的实例方法，并将其添加到分类的方法列表中。
        ...
        //TODO:注意：分类有多个的时候，取决于分类的编辑顺序，即最后编译的分类中的同名方法，才会生效。
    }
    //获取cls中的rw信息，其中包含cls的方法列表的信息。
    auto rw = cls->data();

    //主要是对分类中有关于内存管理的相关方法情况下的 一些特殊处理。
